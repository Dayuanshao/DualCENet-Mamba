...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        loss_hvi = L1_loss(output_hvi, gt_hvi) + D_loss(output_hvi, gt_hvi) + E_loss(output_hvi,
                                                                                     gt_hvi) + opt.P_weight * \
                   P_loss(output_hvi, gt_hvi)[0]
...
def load_datasets():
    ...
    if opt.lol_v1 or opt.lol_blur or opt.lolv2_real or opt.lolv2_syn or opt.SID or opt.SICE_mix or opt.SICE_grad:
        if opt.lol_v1:
            ...
            training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=8,
                                              shuffle=opt.shuffle)
